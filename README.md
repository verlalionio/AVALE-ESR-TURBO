...
---
что за оракулы 🔮
оракулы это кароче говоря сжатые словари данных что бы наш кодировщик мог насытится и перестать расти в размере
есть 2 вида оракулов 

local и base 
local - пополняется каждую загрузку ускоряет все и тому подобное
base - применяется на айди под номером 2 ну точнее что бы обучить его он нужен как словарь что бы иметь его одного с не измеными данными среди пользователей и с помощью айди 6 передавать другим пользователям
...
4. **ID 2**: Обучить Оракула базовой версии
